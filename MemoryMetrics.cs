...
namespace LoadBalancer
{
    ...
    {
        public static MemoryStat MemoryHealthCheck()
        {
            MemoryMetricsClient memoryMetricsClient = new MemoryMetricsClient();
            double percentUsed = memoryMetricsClient.GetMetrics();

            MemoryStat memoryStat = new MemoryStat();

            if (percentUsed < 80)
                memoryStat.Health = "Healthy";
            else if (percentUsed >= 80 && percentUsed < 90)
                memoryStat.Health = "Degraded";
            else
                memoryStat.Health = "Unhealthy";

            memoryStat.LoadPercentage = Math.Round(percentUsed,0);
